...
import NZDateFormatter from './NZDateFormatter';

describe('NZDateFormatter', () => {
  it('renders the correct short date format when length is "short"', () => {
    const testDate = '2024-11-25T12:00:00Z'; // ISO string (UTC)
    
    render(<NZDateFormatter date={testDate} length="short" />);
    
    // Assuming the formatted date will look like: "25 Nov 2024"
    const formattedDate = screen.getByText('25 Nov 2024');
  ...
  it('renders the correct long date format when length is "long"', () => {
    ...
    const formattedDate = screen.getByText('25 November 2024, 12:00:00');
    expect(formattedDate).toBeInTheDocument();
  });
  ...
  it('correctly formats date in Pacific/Auckland timezone', () => {
    ...
    const formattedDate = screen.getByText('25 November 2024, 13:00:00');
    expect(formattedDate).toBeInTheDocument();
  });
  ...
  it('correctly renders short format for edge cases (e.g., leap years)', () => {
    ...
    render(<NZDateFormatter date={testDate} length="short" />);
    const formattedDate = screen.getByText('29 Feb 2024');
    expect(formattedDate).toBeInTheDocument();
  });
});
